...
var (
	helpStr = `wxGifHelper 是一个帮助将Telegram表情包发送至微信的小工具，配合微信小程序使用，小程序扫描二维码，或者搜索小程序处搜索：表情转发助手 即可添加小程序需要绑定微信号才能使用，绑定后即可使用以下命令，你的绑定号是: %d
...
func (m *Msg) Handler() {
	...
	if m.Message.Entities != nil {
		...
		case 1:
			SendText(m.Message, "表情本次已存在，请勿重复发送")
		case 2:
...
func commndHandler(m *Msg, userMsgStatus *common.MsgStatus) {
	...
	case "/stop":
		...
		stopSendStr := fmt.Sprintf("已发送 %d 个表情 ,等待后台处理", len(userMsgStatus.File))
